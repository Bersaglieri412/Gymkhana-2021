...
def iniciarServer(serv_sock):
    serv_sock.bind(('10.20.10.9', 10001))
    sock= socket.socket()
    sock.connect(("rick",8002))
    sock.sendall(id+b' 10001')
    #print(recvall(sock).decode())
    while 1:
        print("Hola")
        recv, client_sock = serv_sock.recvfrom(4096)
        _thread.start_new_thread(listen,(client_sock,))
            
            



def listen(client_sock,rfc):
    print("abro hilo") 
    recv=(recvall(client_sock))
    if(recv.find(b'identifier:')!=-1):
           print("paro")
           print(recv.decode())
    else:
...
serv_sock = socket.socket(socket.AF_INET,socket.SOCK_DGRAM,proto=0)
iniciarServer(serv_sock)
