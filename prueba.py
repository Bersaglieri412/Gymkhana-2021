...
def iniciarServer(serv_sock):
    serv_sock.bind(('10.20.10.9', 10002))
    sock= socket.socket()
    sock.connect(("rick",8002))
    serv_sock.listen(3)
    sock.sendall(id+b' 10002')
    i=0
    while 1:    
        ...
        if(recv.find(b'identifier:')!=-1):
           ...
           print(recv.decode())
            ...
            print(recv.decode())
            print(i)
            _thread.start_new_thread(listen,(client_sock,rfc,))
...
def listen(client_sock,rfc):
    ...
    #print(recvall(client_sock).decode())
    client_sock.sendall(get.text.encode("utf-8"))  
    
    
    #s.sendall(b'GET / HTTP/1.1\r\nHost:http://rick:81/rfc/rfc'+rfc+b'\r\n')
    
    #print(s.recv(1024))
    #s.sendall(get.text.encode())
    #print(s.recv(1024))
    #print("enviado")
    #s.close
